...
Lambda-Emacs (𝛌-Emacs) is intended to serve as a useful, if opinionated, starting point for your own custom configuration of [[https://www.gnu.org/software/emacs/][Emacs]]. The design of this configuration is aimed at writing and academic work in the humanities (for discussion of my view of the use of text editors in the humanities [[https://www.colinmclear.net/posts/texteditor/][see here]]), rather than opposed to, e.g., a programming language emphasis, though I've tried to make it user-friendly for programmers as well. Another goal of 𝛌-Emacs is to provide enough commentary on the configuration such that it might be useful to those new to emacs or even [[https://en.wikipedia.org/wiki/Text_editor][text-editors]] generally.

*NOTE*: Once Emacs 29 is released, Lambda-Emacs will depend on that version (or higher) going forward. This is due to changes in the way Lambda-Emacs installs packages (using the built-in package.el and package-vc.el rather than straight). If you want to try the new system (which also has a number of other improvements) use the branch =package-el=. This branch will be merged to master once Emacs 29 is released. If you're interested in building Emacs 29 from source, see my shell script to easily do so: https://github.com/mclear-tools/build-emacs-macos. 
