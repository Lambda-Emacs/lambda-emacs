...
(use-package project
  ...
  (defun lem-project-try-lsp (dir)
    ...
    ;; Only look for actual LSP markers, not general build files
    (when-let ((root (locate-dominating-file
                      ...
                      (lambda (d)
                        (let ((lsp-files '(".lsp" ".ccls" "compile_commands.json"
                                           ...
                                           "build.gradle")))
                          ;; Also check we're not going too far up
                          (and (not (string= (expand-file-name d) (expand-file-name "~/")))
                               (cl-some (lambda (f)
                                          (file-exists-p (expand-file-name f d)))
                                        lsp-files)))))))
      ;; Only return if the root is reasonable (not home directory)
      (unless (string= (expand-file-name root) (expand-file-name "~/"))
        (cons 'lsp-project root))))
  
  ;; Add LSP detection AFTER VC detection (lower priority)
  (add-to-list 'project-find-functions #'lem-project-try-lsp t)
